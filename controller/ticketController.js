...
const { createEmailTransporter, sendEmail } = require("../utils/sendMail");
const {
  newTicketHtmlTemplate,
} = require("../utils/htmlTemplates/newTicketTemplate");
  ...
  try {
    ...
    const ticket = {
      ...
      image: images
        ? JSON.stringify(
            images.map((image) => ({
              url: path.join("uploads", image.filename),
            })),
          )
        : null,
    ...
    await sendEmail(
      ...
      newTicketHtmlTemplate.html_template,
      values,
      async (succcess) => {
        if (succcess) {
...
const updateTicket = async (req, res) => {
  ...
  const { support_type, support_related_to, title, description, facing_issue_on, status,priority,end_date,assigned_to } = req.body;
  ...
  try {
    ...
    if (req.body.startDate && req.body.endDate) {
      ticketsQuery = ticketsQuery.whereBetween("created_at", [
        `${req.body.startDate} 00:00:00.000000+05:30`,
        `${req.body.endDate} 23:59:59.999999+05:30`,
      ]);
    } else if (req.body.startDate) {
    ...
    return res
      .status(500)
      .json({ error: "An error occurred while getting tickets" });
  }
};
...
const createTicketHistory = async (req, res) => {
  ...
  if (!comment || !ticket_id) {
    return res
      .status(400)
      .json({ message: "Comment and ticket ID are required" });
  }
  ...
  try {
    ...
    return res
      .status(201)
      .json({ message: "Ticket history created", id: result.insertedId });
  } catch (error) {
    console.error(error);
    return res.status(500).json({ message: "Internal server error" });
  }
...
const getTicketHistory = async (req, res) => {
  ...
  try {
    ...
    return res
      .status(500)
      .json({ error: "An error occurred while getting ticket history" });
  }
};
...
const getDistinctSupportTypes = async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ error: "An error occurred while fetching the support types" });
  }
};
...
async function getSubcategories(req, res) {
  try {
    ...
    return res
      .status(500)
      .json({ error: "An error occurred while getting subcategories" });
  }
}
...
async function getFacingIssues(req, res) {
  try {
    ...
    return res
      .status(500)
      .json({ error: "An error occurred while getting issues" });
  }
}
